...
async function seed() {
  ...
  try {
    ...
    await db.insert(schema.roles).values([
      { roleName: "SUPER_ADMIN", description: "Administrator" },  // ✅ FIXED
    ...
    await db.insert(schema.users).values([
      { name: "John Customer", phone: "9876543210", email: "john@example.com", passwordHash, role: "CUSTOMER", status: "ACTIVE" },
      { name: "Meat Master Vendor", phone: "9876543211", email: "vendor@example.com", passwordHash, role: "VENDOR", status: "ACTIVE" },
      { name: "Speedy Rider", phone: "9876543212", email: "rider@example.com", passwordHash, role: "RIDER", status: "ACTIVE" },
      { name: "Admin Super", phone: "9876543213", email: "admin@example.com", passwordHash, role: "SUPER_ADMIN", status: "ACTIVE" }  // ✅ FIXED
    ]).onConflictDoNothing();
    ...
    const admin = allUsers.find(u => u.role === "SUPER_ADMIN");  // ✅ FIXED
    ...
    const [item1, item2] = await db.insert(schema.menuItems).values([
      ...
      { 
        ...
        description: "Perfect for curries",
        price: "180.00", 
        isVeg: false, 
        stockQuantity: 100,
        status: "ACTIVE",
    ...
    await db.insert(schema.orderStatusHistory).values([
      ...
      { orderId: order.id, status: "DELIVERED", remarks: "Delivered successfully" }  // ✅ Changed
    ]);

    // Order Tracking Timeline
    await db.insert(schema.orderTrackingTimeline).values([
      { orderId: order.id, title: "Order Placed", description: "Your order has been placed" },
      { orderId: order.id, title: "Order Accepted", description: "Vendor has accepted your order" },
      { orderId: order.id, title: "Preparing", description: "Your food is being prepared" },
      { orderId: order.id, title: "Ready", description: "Your order is ready for pickup" },
      { orderId: order.id, title: "Out for Delivery", description: "Rider is on the way" },
      { orderId: order.id, title: "Delivered", description: "Order delivered successfully" }
    ]);
    ...
    await db.insert(schema.complaints).values({
      orderId: order.id, 
      customerId, 
      category: "LATE_DELIVERY",  // ✅ Fixed category
      description: "The delivery was delayed by 15 minutes.", 
      status: "OPEN"
    });
    ...
    await db.insert(schema.supportTickets).values({
      customerId, 
      subject: "How to apply coupon codes?", 
      status: "OPEN", 
      priority: "LOW"
    });
